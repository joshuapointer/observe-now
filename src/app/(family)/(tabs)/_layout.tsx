import { Tabs } from "expo-router/tabs";

import { useView } from "@/state/view";
import { Activity, History, MessagesSquare } from "@/ui/icons";
import { AppHeader } from "@/ui/shell";
import { TabBar, type TabItem } from "@/ui/TabBar";
import { useColors } from "@/ui/theme";

// Route names differ from the caregiver tabs' so the two groups never share a URL.
export default function FamilyTabs() {
  const V = useView(), c = useColors();
  const items: TabItem[] = [
    { name: "family", label: "Updates", icon: Activity },
    { name: "family-messages", label: "Messages", icon: MessagesSquare, badge: V.unreadMsgs },
    { name: "history", label: "History", icon: History },
  ];
  return (
    <Tabs
      tabBar={p => <TabBar {...p} items={items} />}
      screenOptions={{ header: () => <AppHeader />, sceneStyle: { backgroundColor: c.page }, animation: "shift" }}
    >
      {items.map(it => <Tabs.Screen key={it.name} name={it.name} options={{ title: it.label }} />)}
    </Tabs>
  );
}
