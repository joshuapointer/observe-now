...
import { useApp, useNow, type AppState } from "./app";
...
export function compute(S: ViewInput, now: number) {
  ...
  const latest = all.filter(e => M.entryCodes(e).length).sort((a, b) => b.markedAt - a.markedAt)[0] || null;
  ...
  const live = family.filter(m => (S.presence.find(x => x.id === m.id)?.lastSeen || 0) > now - 90000);
  const meds = p.meds || [];
  ...
  // Messages go both ways; only family's count as unread for caregivers.
  ...
  const unreadIds = new Set(threads.flatMap(t => [t.root, ...t.replies].filter(n => !fromCaregiver(n) && n.at > readAt(t)).map(n => n.id)));
  ...
  return {
    ...
    now, info, D, PD, day, ctx, d, key, targetIdx, latest, alerts, members, family, live, meds, onShift, roster: S.roster || [],
...
export const nowText = (e: Entry | null, V: View, plain: boolean) => {
  ...
  return codes.length ? codes.map(c => M.codeText(c, V.ctx, plain)).join(", ") : "Nothing recorded yet";
};
